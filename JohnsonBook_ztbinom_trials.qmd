...
r_ztbinom_book <- function(n, N, p) {
  ...
  # min = cdf_ztbinom_book(k = 1, N = N, p = p)
  min = 0
  max = cdf_ztbinom_book(k = upper, N = N, p = p) # can do e^huge
  draws <- rep(0, n)
...
# pdf <- d_ztbinom_book(k = seq(1, N*10, 1), N = N, p = p)
# 
# pdf_df <- as.data.frame(pdf)
# 
# pdf_df <- pdf_df |>
...
#   simulated_data <- r_ztbinom_book(n = n_samples, N = N_fixed, p = current_p)
# 
...
run_diagnostic <- function(t_mean, t_var) {
  ...
  sim_values <- r_ztbinom_book(n = 50000, N = est_n, p = est_p)
